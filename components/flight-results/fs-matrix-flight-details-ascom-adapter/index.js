 ...
 */
class FSMatrixFlightDetailsASComAdapter extends LitElement {
  ...
  unMarshal() {
    this.minifiedModel = [];
    this.minifiedModel = this.formatSegments(this.model.Segments);
  }
  ...
  formatSegments(segments) {
    ...
    segments.map((segment) => {
      result.push({
        ...
        OperationalDisclosures: this.formatOperationalDisclosures(segment, this.model.Footnotes),
    ...
    this.Duration = 'Total Duration: ' + this.formatDuration(this.model.Duration);
    return result;
  }
  ...
  render() {
    return html`
      ${isIE11() ?
        html`<style>
            ${unsafeCSS(FSMatrixFlightDetailsASComAdapter.styles)}
          </style>` :
        ``}
      <div>
        ${this.minifiedModel ?
          html`
          <fs-matrix-flight-details Segments=${JSON.stringify(this.minifiedModel)} 
          Duration=${this.Duration}></fs-matrix-flight-details>
            ` :
          ``}
      </div>
    `;
...
if (!customElements.get('fs-matrix-flight-details-ascom-adapter')) {
  customElements.define('fs-matrix-flight-details-ascom-adapter', FSMatrixFlightDetailsASComAdapter);
}
